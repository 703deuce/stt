  ...
  const handleCompleteTranscription = async (fileToUpload: File) => {
    try {
      ...
      if (result.jobId) {
        ...
        // NOTE: Firestore record creation is now handled by /api/transcribe-complete
        // The API will create the record with proper rate limiting, priority, and tracking
        console.log('✅ Job submitted to API - Firestore record created by API with ID:', result.firestoreRecordId);
