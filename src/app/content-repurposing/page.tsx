...
export default function ContentRepurposingPage() {
  ...
  const extractTranscriptFromData = (data: any): string => {
    ...
    console.log('🧾 [ContentRepurposingPage] Candidate transcript fields:', candidateSources.map(source => ({
      key: source.key,
    ...
    for (const candidate of candidateSources) {
      if (typeof candidate.value === 'string') {
        ...
        console.log(`🧾 [ContentRepurposingPage] Checking candidate "${candidate.key}":`, {
          normalizedLength: normalized.length,
          firstChars: normalized.substring(0, 80),
        });
        if (hasContent(normalized)) {
          console.log(`✅ [ContentRepurposingPage] Using transcript source "${candidate.key}" with length ${normalized.length}`);
          return normalized;
        }
      }
    }
    ...
    if (segmentTranscript) {
      console.log('✅ [ContentRepurposingPage] Using transcript from segment_timestamps');
    ...
    if (timestampTranscript) {
      console.log('✅ [ContentRepurposingPage] Using transcript from timestamps array');
    ...
    if (wordTranscript) {
      console.log('✅ [ContentRepurposingPage] Using transcript from word_timestamps array');
      return wordTranscript;
    }

    console.warn('⚠️ [ContentRepurposingPage] No usable transcript found in data.');
    return '';
  };
      ...
      if (selectedTranscription.transcription_data_url) {
        try {
          ...
          if (fullData?.runpod_output) {
            ...
            console.log('🧾 [ContentRepurposingPage] runpod_output metadata keys:', typeof fullData.runpod_output.metadata === 'object' ? Object.keys(fullData.runpod_output.metadata) : fullData.runpod_output.metadata);
            ...
            console.log('🧾 [ContentRepurposingPage] runpod_output.formatted_transcript preview:', typeof fullData.runpod_output.formatted_transcript === 'string' ? fullData.runpod_output.formatted_transcript.substring(0, 100) : fullData.runpod_output.formatted_transcript);
            console.log('🧾 [ContentRepurposingPage] runpod_output.final_transcript preview:', typeof fullData.runpod_output.final_transcript === 'string' ? fullData.runpod_output.final_transcript.substring(0, 100) : fullData.runpod_output.final_transcript);
            console.log('🧾 [ContentRepurposingPage] runpod_output.metadata.full_transcript preview:', typeof fullData.runpod_output.metadata?.full_transcript === 'string' ? fullData.runpod_output.metadata.full_transcript.substring(0, 100) : fullData.runpod_output.metadata?.full_transcript);
            console.log('🧾 [ContentRepurposingPage] runpod_output.metadata.formatted_transcript preview:', typeof fullData.runpod_output.metadata?.formatted_transcript === 'string' ? fullData.runpod_output.metadata.formatted_transcript.substring(0, 100) : fullData.runpod_output.metadata?.formatted_transcript);
            console.log('🧾 [ContentRepurposingPage] runpod_output.transcript charCodes:', typeof fullData.runpod_output.transcript === 'string' ? Array.from(fullData.runpod_output.transcript.substring(0, 40)).map(ch => ch.charCodeAt(0)) : fullData.runpod_output.transcript);
            console.log('🧾 [ContentRepurposingPage] runpod_output.formatted_transcript charCodes:', typeof fullData.runpod_output.formatted_transcript === 'string' ? Array.from(fullData.runpod_output.formatted_transcript.substring(0, 40)).map(ch => ch.charCodeAt(0)) : fullData.runpod_output.formatted_transcript);
            console.log('🧾 [ContentRepurposingPage] runpod_output.final_transcript charCodes:', typeof fullData.runpod_output.final_transcript === 'string' ? Array.from(fullData.runpod_output.final_transcript.substring(0, 40)).map(ch => ch.charCodeAt(0)) : fullData.runpod_output.final_transcript);
            console.log('🧾 [ContentRepurposingPage] runpod_output.metadata.full_transcript charCodes:', typeof fullData.runpod_output.metadata?.full_transcript === 'string' ? Array.from(fullData.runpod_output.metadata.full_transcript.substring(0, 40)).map(ch => ch.charCodeAt(0)) : fullData.runpod_output.metadata?.full_transcript);
          }
